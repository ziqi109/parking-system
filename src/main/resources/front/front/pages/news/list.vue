...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"linear-gradient(0deg, rgba(255,255,255,1) 80%, rgba(34,56,96,1) 98%, rgba(34,56,96,1) 100%),#fff","flexDirection":"column","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"padding":"0px","margin":"0 auto","borderRadius":"16rpx","background":"none","display":"flex","width":"calc(100% - 48rpx)","height":"auto"}'>
				...
				<view :style='{"margin":"0","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 80rpx","color":"#333","borderRadius":"16rpx 0 0 16rpx","background":"#fff","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.title" type="text" placeholder="标题" ></input>
				</view>
			...
			<view :style='{"padding":"20rpx 0","margin":"260rpx 0 0 0","borderRadius":"60rpx 60rpx 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","lineHeight":"120rpx","position":"absolute","justifyContent":"center","order":"2","height":"120rpx"}'>
				<view @click="sortClick('clicknum')" :style='{"border":"0","padding":"0 12rpx","margin":"0 8rpx 0 0","borderRadius":"8rpx","background":"none","display":"flex"}'>
					<text :style='{"color":"#333","lineHeight":"100rpx","fontSize":"28rpx"}'>点击量</text>
					<text v-if="listSort!='clicknum'" class="icon iconfont icon-jiantou36" :style='{"margin":"0 4rpx 0 0","lineHeight":"100rpx","fontSize":"28rpx","color":"#333"}'></text>
					<text v-else-if="listSort=='clicknum'&&listOrder=='asc'" class="icon iconfont icon-jiantou35" :style='{"margin":"0 4rpx 0 0","lineHeight":"100rpx","fontSize":"28rpx","color":"#333"}'></text>
					<text v-else-if="listSort=='clicknum'&&listOrder=='desc'" class="icon iconfont icon-jiantou36" :style='{"margin":"0 4rpx 0 0","lineHeight":"100rpx","fontSize":"28rpx","color":"#333"}'></text>
				</view>
			...
			<view :style='{"alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"10"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"margin":"40rpx 0 0","whiteSpace":"nowrap","background":"none","display":"flex","width":"calc(100% - 48rpx)","justifyContent":"center","height":"auto","order":"0"}'>
				...
				<!-- 样式1 -->
				<view class="news-box1" :style='{"width":"100%","padding":"24rpx","margin":"120rpx 0 0","background":"#fff","height":"auto","order":"50"}'>
					<view @tap="onDetailTap(product)" v-for="(product,index) in list" :key="index" class="list-item" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 40rpx","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
					  <view class="list-item-body" :style='{"padding":"8rpx 0 0 20rpx","margin":"0","background":"linear-gradient(30deg, rgba(246,249,255,1) 0%, rgba(239,244,255,1) 100%),#f1f6ff","clipPath":"polygon(4% 0, 100% 0%, 96% 100%, 0% 100%)","width":"calc(100% - 200rpx)","height":"220rpx","order":"2"}'>
						<view :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"60rpx","fontSize":"28rpx","color":"#333"}' class="title ">{{product.title}}</view>
						<view :style='{"padding":"0 20rpx","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#666","width":"100%","lineHeight":"1.5","fontSize":"28rpx","textOverflow":"ellipsis"}' class="text ">{{product.introduction}}</view>
						<view :style='{"padding":"0 10rpx","margin":"0 10rpx 0 0"}'>
					  ...
					  <image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"8rpx","display":"block","clipPath":"polygon(10% 0, 100% 0%, 90% 100%, 0% 100%)","height":"220rpx"}' mode="aspectFill" class="listmpic" :src="baseUrl+product.picture"></image>
					</view>
				</view>
		...
		<button :style='{"border":"0","boxShadow":"0 0px 4rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#ff6b5a","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('news','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 4rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#ff6b5a","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('news','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
