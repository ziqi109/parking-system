<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px 0","fontSize":"14px","color":"#666","background":"#f6f6f6"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					...
					<el-button class="btn18" v-if="isAuth('cheliangchuchang','收入情况')" type="success" @click="chartDialog1()">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="hejifeiyongChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var hejifeiyongChart1 = echarts.init(document.getElementById("hejifeiyongChart1"),'macarons');
        this.$http({
            url: `cheliangchuchang/value/dengjiriqi/hejifeiyong`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                hejifeiyongChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    hejifeiyongChart1.resize();
                };
            }
        });
      })
